...
class sää:
    ...
    def setup_window_geometry(self):          # ikkunan geometria, näitä voidaan viilailla
        ...
        y_position = (screen_width // 6) - (window_width // 6) # mulla ultrawide näyttö, eli joudutte näitä muokkaamaan että teillä keskellä ruutua 
    ...
    def setup_grid(self):
        for i in range(4):              # 4 rows (ylähäältä alaspäin)
            self.app.grid_rowconfigure(index=1, weight=1)
        for j in range(5):              # 5 columns (vasemmalta oikealle)
            self.app.grid_columnconfigure(index=j, weight=1)
        
        
    def create_widgets(self):               # sitten meidän widgetit, eli päivän säät yms
        päivän_sää = ttk.Label(self.app, text="Päivän sää", bootstyle="warning")
        päivän_sää.grid(row=0, column=0, columnspan=5, sticky="nsew")

        paikkakunta = ttk.Label(self.app, text="Syötä Paikkakunta", bootstyle="primary")
        paikkakunta.grid(row=1, column=0, columnspan=2, sticky="nsew")

        kolmen_paivan = ttk.Label(self.app, text="3 päivän sää", bootstyle="success")
        kolmen_paivan.grid(row=2, column=2, columnspan=3, sticky="nsew")
        ...
        self.let_entry.grid(row=3, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")
        ...
        b2.grid(row=3, column=4, padx=10, pady=10, sticky="e")
