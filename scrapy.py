...
def show_input(app):
    user_input = city_entry.get()            # hakee käyttäjän syötteen
    
def on_entry_click(event):                  # Piilotetaan paikkakunta-txt
    if city_entry.get() == placeholder_text:
        city_entry.delete(0, ttk.END)         # tyhjentää
        city_entry.config(bg="white")  # vaihdetaan txt mustaksi tyhjennyksen jälkeen, huom bootstrapin omat värikoodit
        
def on_focusout(event):
    if city_entry.get() == "":
        city_entry.insert(0, placeholder_text) # tuodaan "SYötä paikkakunta" takaisin
        city_entry.config(bg='lightgray') # Secondary = harmaa//vaihdettu
...
city_entry.bind("<Button-1>", on_entry_click) # hiiren painalluksella
city_entry.bind("<FocusOut>", on_focusout)   # pitäis txt tulla takaisin kun klikkaa pois, ei ilmeisesti toimi vielä
